...
def test_a_list_shows_only_your_own_saves(client, auth, other_client, other_auth):
    ...
    mine = client.get("/api/v1/bookmarks", headers=auth).json()
    assert mine["total"] == 1
...
def test_you_cannot_fetch_another_users_save_by_id(client, auth, other_client, other_auth):
    ...
    assert (
        client.get(f"/api/v1/bookmarks/{theirs['id']}", headers=auth).status_code
...
def test_you_cannot_modify_another_users_save(client, auth, other_client, other_auth):
    ...
    assert client.patch(
        f"/api/v1/bookmarks/{theirs['id']}", json={"title": "hijacked"}, headers=auth
    ).status_code == status.HTTP_404_NOT_FOUND

    assert client.delete(
        f"/api/v1/bookmarks/{theirs['id']}", headers=auth
    ).status_code == status.HTTP_404_NOT_FOUND

    assert client.post(
        f"/api/v1/bookmarks/{theirs['id']}/tags", json={"tags": ["x"]}, headers=auth
    ).status_code == status.HTTP_404_NOT_FOUND
...
def test_lookup_does_not_reveal_that_someone_else_saved_a_page(
    ...
    r =client.get(
        "/api/v1/bookmarks/lookup", params={"url": "https://example.com/theirs"}, headers=auth
    )
...
def test_deleting_your_save_leaves_the_other_users_alone(
    ...
    client.delete(f"/api/v1/bookmarks/{mine['id']}", headers=auth)

    assert client.get("/api/v1/bookmarks", headers=auth).json()["total"] == 0
    assert other_client.get("/api/v1/bookmarks", headers=other_auth).json()["total"] == 1
